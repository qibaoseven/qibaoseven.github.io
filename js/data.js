...
const STORAGE_KEYS = {
    ...
    DAILY_CHECK: 'studentos_daily_check',  // 新增：每日检查记录
    AUTO_EVENTS: 'studentos_auto_events',  // 新增：自动事件
...
async function loadFromCloud() {
    try {
        ...
        if (cloudData.dailyCheck) window.appData.dailyCheck = cloudData.dailyCheck;
...
async function saveToCloud() {
    try {
        const uploadData = {
            ...
            dailyCheck: window.appData.dailyCheck,
...
async function checkDailyReports() {
    ...
    const dailyCheck = window.appData.dailyCheck || {};
    ...
    for (const date of allDates) {
        ...
        if (!hasAllCheck) {
            ...
            admins.forEach(adminId => {
                try {
                    if (window.appData.scores && window.appData.scores[adminId]) {
                        window.appData.scores[adminId][1] = (window.appData.scores[adminId][1] || 0) - 5;
                        window.utils.addLog('每日检查扣分', adminId, -5, window.appData.scores[adminId][1], `日期 ${date} 未完成全员检查`);
                    }
                } catch (e) {
                    console.warn(`扣分失败: admin ${adminId}`, e);
                }
            });
        }
    ...
    window.appData.dailyCheck = dailyCheck;
    saveData('dailyCheck');
    await saveToCloud();
}
